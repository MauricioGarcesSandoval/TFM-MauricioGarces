...
if __name__ == '__main__':
    ...
    print("#########################################################################")
    # df_union.persist().show(5)
    # df_union.select("year", "month", "day").distinct().show(20, truncate=False)
    ...
    df_memory_errors = df_exploded \
        ...
        .filter("host like 'login%' or host like 'c20%' or host like 'c21%' or host like 'hsm212%' or host like 'osshsm213%' or host like 'oss21%'") \
    ...
    df_memory_errors.write.mode('overwrite').option('compression', 'snappy').partitionBy('year', 'month', 'day', 'host').parquet('memory-errors')
